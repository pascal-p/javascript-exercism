...
describe('Meetup', () => {
  ...
  test('wednesteenth of January 2013', () => {
    expect(meetup(2013, 1, 'teenth', 'Wednesday')).toEqual(
      new Date(2013, 0, 16)  // Wed 16 Jan 2013
    );
  });
...
describe('Meetup Exception', () => {

  test('incorrect year/1', () => {
    expect(() => {
      meetup('101', 12, 'last', 'Sunday');
    }).toThrow(new Error('Expecting year to be an integer >= 2000'));
  });

  test('incorrect year/2', () => {
    expect(() => {
      meetup(101, 12, 'last', 'Sunday');
    }).toThrow(new Error('Expecting year to be an integer >= 2000'));
  });

  test('incorrect year/3', () => {
    expect(() => {
      meetup(true, 12, 'last', 'Sunday');
    }).toThrow(new Error('Expecting year to be an integer >= 2000'));
  });

  test('incorrect month/1', () => {
    expect(() => {
      meetup(2010, 0, 'last', 'Sunday');
    }).toThrow(new Error('Expecting month to be an 1 <= integer <= 12'));
  });

  test('incorrect month/2', () => {
    expect(() => {
      meetup(2010, 'May', 'last', 'Monday');
    }).toThrow(new Error('Expecting month to be an 1 <= integer <= 12'));
  });

  test('incorrect month/3', () => {
    expect(() => {
      meetup(2010, 'May', 'last', 'Monday');
    }).toThrow(new Error('Expecting month to be an 1 <= integer <= 12'));
  });

  test('incorrect descriptor/1', () => {
    expect(() => {
      meetup(2010, 5, 'foo', 'Sunday');
    }).toThrow(new Error('Expecting descriptor to be one of: first,second,third,fourth,fifth,last,teenth'));
  });

  test('incorrect descriptor/2', () => {
    expect(() => {
      meetup(2010, 5, '1st', 'Sunday');
    }).toThrow(new Error('Expecting descriptor to be one of: first,second,third,fourth,fifth,last,teenth'));
  });

  test('incorrect weekday/1', () => {
    expect(() => {
      meetup(2010, 5, 'first', 'Lundi');
    }).toThrow(new Error('Expecting weekday to be one of: Sunday,Monday,Tuesday,Wednesday,Thursday,Friday,Saturday'));
  });

  test('incorrect weekday/2', () => {
    expect(() => {
      meetup(2010, 5, 'last', 'Dimanche');
    }).toThrow(new Error('Expecting weekday to be one of: Sunday,Monday,Tuesday,Wednesday,Thursday,Friday,Saturday'));
  });
