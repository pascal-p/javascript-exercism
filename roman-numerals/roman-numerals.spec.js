...
describe('toRoman()', () => {
  ...
  test('converts 10', () => expect(toRoman(10)).toEqual('X'));
  ...
  test('converts 1703', () => expect(toRoman(1703)).toEqual('MDCCIII'));
  test('converts 1991', () => expect(toRoman(1991)).toEqual('MCMXCI'));
  test('converts 3000', () => expect(toRoman(3000)).toEqual('MMM'));

  test('cannot convert 0', () => {
    expect(() => expect(toRoman(0)).toThrow(new Error('Out of range')));
  });
  test('cannot convert -1', () => {
    expect(() => expect(toRoman(-1)).toThrow(new Error('Out of range')));
  });
  test('cannot convert foo', () => {
    expect(() => expect(toRoman(-1)).toThrow(new Error('NaN: Not a Number!')));
  });
});
